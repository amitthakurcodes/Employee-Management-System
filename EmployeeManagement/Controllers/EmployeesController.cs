...
namespace EmployeeManagement.Controllers
{
    public class EmployeesController : Controller
    {
        ...
        public IActionResult Grouping(string groupby)
        {
            if (groupby == "gender")
            {
                var data = db.Employees
                    .GroupBy(x => x.Gender)
                    .ToList()
                    .Select(g => new GroupData<Employee>
                    {
                        Key = g.Key.ToString(),
                        Persons = g
                    }).ToList();
            ...
            else if (groupby == "year-month")
            {
                var data = db.Employees
                    .ToList()
                    .GroupBy(x => new { x.JoiningDate.Value.Year, x.JoiningDate.Value.Month })
                    .Select(g => new GroupData<Employee>
                    {
        ...
        public IActionResult Create(EmployeeInputModel model, string operation)
        {
            ...
            if (operation != null && operation.StartsWith("del"))
            {
                int index = int.Parse(operation.Substring(operation.IndexOf("_") + 1));
                model.Attendances.RemoveAt(index);
                ...
                if (ModelState.IsValid)
                {
                    ...
                    foreach (var a in model.Attendances)
                    {
                        db.Database.ExecuteSqlInterpolated(
                            $"EXECUTE InsertAttendance {a.AttendanceDate}, {a.InTime}, {a.OutTime}, {e.EmployeeId}"
                        );
                    }
        ...
        public IActionResult Edit(EmployeeEditModel model, string operation)
        {
            ...
            if (operation == "update")
            {
                if (ModelState.IsValid)
                {
                    ...
                    db.Database.ExecuteSqlInterpolated(
                        $"EXEC DeleteAttendanceOfEmployee {e.EmployeeId}"
                    );
        ...
        public IActionResult Delete(int id)
        {
            if (!db.Employees.Any(x => x.EmployeeId == id))
                return NotFound();
